...
def getDuelStatsTimeline(matchList):
    events = []
    ...
    for event in events:
        ...
        if event.getMinute() >= timeframeEnd and event.getMinute() > 0:
            ...
            airBwon = 0
        if typeId == "3":
            if teamId == matchList[0] and outcome == "0":
                groundBwon += 1
            elif teamId != matchList[0] and outcome == "1":
                groundBwon += 1
            elif teamId == matchList[0] and outcome == "1":
                groundAwon += 1
            elif teamId != matchList[0] and outcome == "0":
                groundAwon += 1
        elif typeId == "7":
            if teamId == matchList[0]:
                groundAwon += 1
            else:
                groundBwon += 1
        elif typeId == "44":
...
def printDuelsTimelineMultiple(matchList):
    groundA, groundB, airA, airB, time, timeInterval = getDuelStatsTimeline(matchList)

    groundRange = range(0, ((max(groundA) + max(groundB)) + 1), 2*len(matchList[1:]))
    airRange = range(0, ((max(airA) + max(airB)) + 1), len(matchList[1:]))
    yticks = []
    for times in time:
